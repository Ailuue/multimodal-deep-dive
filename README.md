...
question — that's the slot. See [examples/02_vision_describe.py](examples/02_vision_describe.py);
...
| Extract structured data from screenshots | **Either** | It's a vision call + a JSON-shaped prompt — provider-agnostic |
...
| The transcript is empty | The bundled `note.wav` is a tone, not speech — there's nothing to transcribe. Point it at a real voice recording. |
